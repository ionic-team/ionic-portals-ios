...
- (void)testPubSub__when_provided_a_simple_json_compatible_value__it_can_be_coerced_correctly {
    ...
    NSInteger subRef = [IONPortalsPubSub subscribeToTopic:@"test" callback:^(NSDictionary<NSString *,id> * _Nonnull dict) {
        NSNumber *integer = dict[@"data"];
        XCTAssertEqual(integer, @1);
        [expectation fulfill];
    }];
    
    [IONPortalsPubSub publishToTopic:@"test" data:@1];
    
    [self waitForExpectations:@[expectation] timeout:5.0];
    [IONPortalsPubSub unsubscribeFromTopic:@"test" subscriptionRef:subRef];
}

- (void)testPubSub__when_provided_a_non_json_compatible_value__it_cannot_be_coerced_correctly {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback should have fired"];
    ...
    NSPredicate *nonJsonCompatible = [[NSPredicate alloc] init];
...
- (void)testPubSub__when_provided_a_json_compatible_nsdictionary__it_correctly_coerces_it {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback should have fired"];
...
- (void)testPubSub__when_provided_an_nsdictionary_with_objects_incompatible_with_json__it_only_coerces_valid_data {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback should have fired"];
    ...
    NSDictionary *aDictToPublish = @{
        ...
        @"bool": @YES,
        @"null": [NSNull null],
        @"incompatiblePredicate": [[NSPredicate alloc] init],
...
- (void)testPubSub__when_provided_a_json_compatible_nsarray__it_correctly_coerces_it {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback should have fired"];
...
- (void)testPubSub__when_provided_an_nsarray_with_incompatible_elements__incompatible_elements_are_ignored {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback should have fired"];
