...
public class PortalUIView: UIView {
    ...
    func initView () {
        if PortalsRegistrationManager.shared.isRegistered {
            ...
            let unregisteredView = Unregistered(shouldShowRegistrationError: showRegistrationError)
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .edgesIgnoringSafeArea(.all)
                .background(Color.portalBlue)

            let view = UIHostingController(rootView: unregisteredView).view
    ...
    final class InternalCapWebView: CAPWebView {
        ...
        override func instanceDescriptor() -> InstanceDescriptor {
            ...
            let capConfigUrl = portal.bundle.url(
                forResource: "capacitor.config",
                withExtension: "json",
                subdirectory: portal.startDir
            )

            let cordovaConfigUrl = portal.bundle.url(
                forResource: "config",
                withExtension: "xml",
                subdirectory: portal.startDir
            )
            ...
            let descriptor = InstanceDescriptor(
                at: path,
                configuration: capConfigUrl,
                cordovaConfiguration: cordovaConfigUrl
            )
